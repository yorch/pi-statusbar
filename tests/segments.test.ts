  ...
  for (const [name, def] of Object.entries(PRESETS)) {
    for (const row of def.rows) {
      for (const id of [...row.left, ...(row.right ?? [])]) {
        assert.ok((SEGMENTS as Record<string, unknown>)[id], `preset "${name}" references unknown segment "${id}"`);
      }
    }
  }
});
...
test('context segment fallback and thresholds', () => {
  ...
  const fallback = SEGMENTS.context.render(
    createSegmentContext({
      ctxOverrides: {
        getContextUsage: () =>
          null as unknown as ReturnType<import('@earendil-works/pi-coding-agent').ExtensionContext['getContextUsage']>,
      },
    }),
  );
...
test('git segment clean vs dirty and upstream', () => {
  ...
  const dirty = SEGMENTS.git.render(createSegmentContext({ git: makeGit({ branch: 'feat', staged: 1, unstaged: 2 }) }));
